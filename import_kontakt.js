...
function cleanItem(rawTitle) {
  ...
  } else if (lstr.includes('ethnic') || lstr.includes('ireland') || lstr.includes('west africa') || lstr.includes('arabian') || lstr.includes('harp') || lstr.includes('koto') || lstr.includes('ukulele')) {
...
// Convert to tier items
const kontaktItems = parsed.map(p => {
  return {
...
// Update data/windows.js and data/mac.js
['./data/windows.js', './data/mac.js'].forEach(file => {
  ...
  console.log(`Updated ${file} with ${kontaktItems.length} Kontakt libraries!`);
});
